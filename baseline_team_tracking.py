...
class ReflexCaptureAgent(CaptureAgent):
    ...
    def __init__(self, index, inferences, inference_initializer=False):
        """Initialize agent with given inferences lists."""
    ...
    def register_initial_state(self, game_state):
        ...
        if self.inference_initializer:
            self.inferences[:] = [ExactInference(opponent, self.distancer,
                                                 game_state) for opponent in
    ...
    def choose_action(self, game_state):
        ...
        self.display_distributions_over_positions(
                                        [inference.get_belief_distribution()
                                         for inference in self.inferences])
        actions = game_state.get_legal_actions(self.index)

        # You can profile your evaluation time by uncommenting these lines
        # start = time.time() (must import time)
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def get_features(self, game_state, action):
        ...
        invader_indices = [i for i in self.get_opponents(successor)
        ...
        if len(invaders) > 0:
            ...
            for index in invader_indices:
                invader_inferences += [inference
                                       for inference in self.inferences
                                       if inference.index == index]
            dists =\
                [self.get_maze_distance(my_pos,
                                        inference.get_most_likely_position())
