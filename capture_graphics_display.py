"""Graphical display for Pacman capture the flag.
...
class InfoPane:
    ...
    def __init__(self, layout, grid_size, red_team, blue_team):
        """Create InfoPane based on given layout, grid_size, and teams."""
    ...
    def draw_pane(self):
        """Draw the pane to screen."""
        self.score_text = gu.text(self.to_screen(0, 0), self.text_color,
                                  self._info_string(0, 1200), "Consolas",
                                  self.font_size, "bold")
        self.red_text = gu.text(self.to_screen(0, INFO_PANE_HEIGHT/2), TEAM_COLORS[0],
                                self._red_score_string(), "Consolas",
                                self.font_size, "bold")
        self.blue_text = gu.text(self.to_screen(self.width/2, 
                                                INFO_PANE_HEIGHT/2), 
                                 TEAM_COLORS[1],
                                 self._blue_score_string(), "Consolas",
                                 self.font_size, "bold")

    def _red_score_string(self):
        return "RED: % 10s " % (self.red_team[:20])

    def _blue_score_string(self):
        return "BLUE: % 10s " % (self.blue_team[:20])
...
class PacmanGraphics:
    """Graphics for Pacman Capture The Flag."""

    def __init__(self, red_team, blue_team, zoom=1.0, frame_time=0.0,
                 capture=False):
    ...
    def _start_graphics(self, state):
        ...
        self.info_pane = InfoPane(layout, self.grid_size, self.red_team,
                                  self.blue_team)
    ...
    def make_window(self, width, height):
        ...
        gu.begin_graphics(screen_width, screen_height, BACKGROUND_COLOR,
                          "CSI-480 Pacman Capture The Flag")
    ...
    def move_ghost(self, ghost, ghost_index, prev_ghost, ghost_image_parts):
        ...
        for ghost_image_part in ghost_image_parts:
            gu.move_by(ghost_image_part, delta, lift=True)
        gu.refresh()
