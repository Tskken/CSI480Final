...
class Layout:
    ...
    def initialize_visibility_matrix(self):
        """Initialize the visibility matrix using the global cache."""
        global VISIBILITY_MATRIX_CACHE
        if (reduce(str.__add__, self.layout_text) not in
                VISIBILITY_MATRIX_CACHE):
    ...
    def is_wall(self, pos):
        """Return whether position is wall or not."""
        x, col = pos
        return self.walls[x][col]

    def get_random_legal_position(self):
        """Return a random legal position."""
    ...
    def get_furthest_corner(self, pac_pos):
        """Return the furhest corner from pacman (manhattan distance)."""
        poses = [(1, 1), (1, self.height - 2), (self.width - 2, 1),
                 (self.width - 2, self.height - 2)]
    ...
    def is_visible_from(self, ghost_pos, pac_pos, pac_direction):
        """Return wheter the ghost is visible by pacman."""
        row, col = [int(x) for x in pac_pos]
        return ghost_pos in self.visibility[row][col][pac_direction]

    def __str__(self):
        """Return string representation of layout_text."""
    ...
    def process_layout_char(self, x, y, layout_char):
        """Process a single layout character at position x,y."""
        if layout_char == '%':
            self.walls[x][y] = True
        elif layout_char == '.':
            self.food[x][y] = True
...
def try_to_load(fullname):
    """Try to load layout based on full path name."""
