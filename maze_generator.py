...
class Maze:
    """A class defining a maze."""

    def __init__(self, rows, cols, anchor=(0, 0), root=None):
        """Generate an empty maze.

        Anchor is the top left corner of this grid's position
        in its parent grid.
        """
        self.r = rows
        self.c = cols
    ...
    def to_map(self):
        """Add a flipped symmetric copy on the right and add a border."""
...
def make_with_prison(room, depth, gaps=1, vert=True, min_width=1,
                     gap_factor=0.5):
...
def make(room, depth, gaps=1, vert=True, min_width=1, gap_factor=0.5):
    ...
    #     vert = not vert
    for sub_room in room.rooms:
        make(sub_room, depth + 1, max(1, gaps * gap_factor), not vert,
             min_width, gap_factor)
    # for sub_room in room.rooms:
...
def add_pacman_stuff(maze, max_food=60, max_capsules=4, toskip=0):
    ...
    Also add food at dead ends plus some extra.
    """
    # parameters
    max_depth = 2
    ...
    total_food = 0
    while True:
        new_grid = copy.deepcopy(maze.grid)
        depth += 1
        num_added = 0
        for row in range(1, maze.r - 1):
            for col in range(1 + toskip, (maze.c // 2) - 1):
                ...
                if maze.grid[row][col] != E:
                    continue
                neighbors = ((maze.grid[row - 1][col] == E) +
...
def generate_maze(seed=None):
    """Generate a maze given random seed."""
    if not seed:
    ...
    skip = make_with_prison(maze, depth=0, gaps=3,
                            vert=True, min_width=1, gap_factor=gap_factor)
    maze.to_map()
